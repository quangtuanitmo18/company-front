<template>
  <app-page-title iconClassname="mdi-account" title="Профиль пользователя" subtitle="Редактировать"></app-page-title>
  <card-with-actions :title="`${user.surname} ${user.firstname} ${user.lastname}`" :actions="configActions">
    <v-form ref="form" class="mt-4">
      ...
      <div v-if="notion || errors.length" class="form-notion mt-4">
        <p v-if="notion" class="form-notion-text" :class="{[notion.status]: true}">{{ notion.text }}</p>
...
import {userDetail, userRoleListByGroupForFilter, userUpdate} from "@/service/auth/users/userService.js";
  ...
  userUpdate(route.params.id, {
    ...
    userDetail(route.params.id).then(res => {
      ...
      user.value = {
        ...
        ossaProjectData: {
          telephoneNumber: res.user.ossaProjectData && res.user.ossaProjectData.telephoneNumber || "",
          mobileNumber: res.user.ossaProjectData && res.user.ossaProjectData.mobileNumber || "",
          sqlupUserIdIgs: res.user.ossaProjectData && res.user.ossaProjectData.sqlupUserIdIgs || "",
          sqlupUserIdCso: res.user.ossaProjectData && res.user.ossaProjectData.sqlupUserIdCso || "",
          sqlupUserIdSsp: res.user.ossaProjectData && res.user.ossaProjectData.sqlupUserIdSsp || "",
          isOperator: res.user.ossaProjectData && res.user.ossaProjectData.isOperator || 0,
          operatorDepartment: res.user.ossaProjectData && res.user.ossaProjectData.operatorDepartment || null
        }
      }
