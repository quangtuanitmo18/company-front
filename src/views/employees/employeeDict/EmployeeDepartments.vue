<template>
  <div class="mx-2 mt-4">
    <div class="d-flex">
      <v-btn class="ml-auto" prepend-icon="mdi-office-building-plus" color="primary" variant="outlined" @click="handleOpenCreate">
        Отдел
      </v-btn>
    </div>
    ...
    <modal v-model="isShowModal" :title="isCreateDepartment ? 'Создание отдела' : 'Редактирование отдела'">
      <template #content>
        <v-form ref="formDepartment" class="mt-4">
          <v-row>
            <v-col cols="12">
              <v-select
                  variant="underlined"
                  v-model="department.enabled"
                  :items="STATUS"
                  label="Активен"
                  hide-details
              >
      ...
      <template #actions>
        ...
        <btn-primary :loading="isLoading" @click="handleSubmit">
          {{ isCreateDepartment ? "Создать" : "Подтвердить" }}
        </btn-primary>
      </template>
    </modal>
    <modal v-model="dialogDeleteConfirm" title="Подтверждение действия" text="Вы действительно хотите удалить отдел?">
      <template #actions>
...
import {
  ...
  employeeDictionaryDelete, employeeDictionaryEdit
...
const handleEdit = (item) => {
  isShowModal.value = true
  isCreateDepartment.value = false
  department.value = JSON.parse(JSON.stringify(item))
}
...
const handleDelete = async () => {
  ...
  await employeeDictionaryDelete("department", department.value.id).then((res) => {
...
const handleSubmit = async () => {
  ...
  if(valid){
    ...
    if(isCreateDepartment.value){
      await employeeDictionaryAdd("department", department.value).then((res) => {
      ...
      await employeeDictionaryEdit("department", department.value.id, department.value).then(res => {
        isShowModal.value = false
      }).catch(err => {
        notification.value = {
          value: true,
