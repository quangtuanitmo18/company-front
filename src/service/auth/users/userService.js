...
import {operatorList} from "@/service/ossa/users/userOperatorsService.js";
...
export const userRoleListByGroupForFilter = async (data) => {
    try{
        ...
        items.forEach((item) => {
            ...
            objByGroup[item.project.title].push({value: item.id, title: item.title, project: item.project.title})
        })
